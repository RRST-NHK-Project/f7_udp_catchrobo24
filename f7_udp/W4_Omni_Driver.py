...
class Listener(Node):
    ...
    def listener_callback(self,ps4_msg):
        ...
        if (math.fabs(LS_X) <= deadzone) and (math.fabs(LS_Y) <= deadzone) and R2 == 0 and L2 == 0:
            v1 = 0
            v2 = 0
            v3 = 0
            v4 = 0
...
class udpsend():
    ...
    def send(self):
        ...
        str_data = (str(data[1])+','+str(data[2])+','+str(data[3])+','+str(data[4])+','+str(data[5])+','+str(data[6])+','+str(data[7])+','+str(data[8])) #パケットを作成
